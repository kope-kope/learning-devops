...
**The Stack:**
- **VMware Desktop** as the hypervisor (on my Mac M4 chip)
...
- Without RabbitMQ: Service A calls Service B directly → If B is down, everything breaks
- With RabbitMQ: Service A sends message to queue → Service B processes when ready
...
- If not (cache miss) → Query database → Store in cache for next time

**Real-world impact:** 
- Without cache: 10,000 users viewing a profile = 10,000 database queries 💥
- With cache: First user hits DB, next 9,999 get cached data (< 1ms each) ✨
...
**After:**
```bash
Available repos: BaseOS, AppStream, Extras, EPEL ← NEW!
```
...
- db01: 600 MB ⚠️ (still low for MySQL)
- mc01: 512 MB (default) ⚠️
- rmq01: 512 MB (default) ⚠️
- app01: 1024 MB ✅
- web01: 512 MB (default) ⚠️
...
- `$` = Bash terminal → Shell commands work
- `mysql>` = MySQL prompt → SQL commands work
...
### 1. Cache Strategy
"Check cache → then database" is a pattern I'll see everywhere. It's not just about speed, it's about **reducing load on the expensive resource** (the database).
